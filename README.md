...
Zero runtime dependencies. Written in functional JavaScript with complete TypeScript definitions (`roomer.d.ts`, `bytecursor.d.ts`, `emitter.d.ts`).
...
const root = roomer("ws://localhost:8080/ws", {
    ...
    initial_delay: 500,  // Initial backoff in ms (Default: 500)
    max_delay: 5000      // Maximum backoff ceiling in ms (Default: 5000)
});
...
root.on("open", () => {
    ...
    lobby.on("new_member", (memberId) => {
        console.log("Member joined lobby:", memberId);
    });

    lobby.on("member_left", (memberId) => {
        console.log("Member left lobby:", memberId);
    });
});
```
...
|---|---|---|
| `.id()` | `string` | Connection UUID assigned by the server. |
| `.open()` | `boolean` | `true` if room membership is currently active. |
| `.members()` | `string[]` | Shallow copy array of all active member IDs in this room. |
| `.join(roomName)` | `Room` | Subscribes to a room channel over the existing connection. |
| `.leave()` | `Room` | Unsubscribes from the room and notifies the cluster. |
| `.send(event, payload?, dst?)` | `Room` | Sends a message packet (broadcast to room, or direct to `dst`). |
| `.clearListeners([exceptions])`| `Room` | Clears registered listeners except those listed in `exceptions`. |
| `.forceClose(isDisconnect?)` | `Room` | Closes room state locally and emits `"close"`. |
| `.purge()` *(root only)* | `Room` | Leaves all non-root rooms simultaneously. |
| `.rooms()` *(root only)* | `Record<string, Room>` | Read-only map of all active room instances. |
